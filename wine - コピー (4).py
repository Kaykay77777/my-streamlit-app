...
if st.session_state.selected_location:
    ...
    if st.button('ワインを登録'):
        ...
        photo_paths = ';'.join(new_photos)
        
        if not existing_wine.empty:
            existing_wine_dict = existing_wine.iloc[0].fillna('').to_dict()
        else:
            existing_wine_dict = {}

        wine_info = pd.DataFrame([{**existing_wine_dict,
                           'ワイン名': wine_name, '年': wine_year, '種類': wine_type, '場所': st.session_state.selected_location,
                           '詳細情報': wine_details, '写真': photo_paths, '購入日': purchase_date, '価格': price, 
                           '購入場所': purchase_place, '国': wine_country, '地域': wine_region, '評価': wine_rating, '抜栓日': opening_date}])
