...
if st.session_state.selected_location:
    ...
    # 抜栓日を取得し、NaTの場合はNoneに変換
    opening_date = existing_wine['抜栓日'].values[0] if not existing_wine.empty else None
    if pd.isna(opening_date):  # NaTをNoneに変換
        opening_date = None
    elif isinstance(opening_date, str):  # 文字列の場合は日付に変換
        try:
            opening_date = pd.to_datetime(opening_date).date()
        except Exception:
            opening_date = None  # 変換できない場合はNoneにする
    ...
    if wine_images:  
        ...
        photo_paths = ';'.join(new_photos) if new_photos else existing_wine["写真"].values[0] if not existing_wine.empty else ""
